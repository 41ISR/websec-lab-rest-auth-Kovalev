...
const app = express()
const SECRET = process.env.SECRET || 'book_api_secret_key'
...
app.post('/api/auth/register', (req, res) => {
    try {
        ...
        if (!email || !username || !password) return res.status(400).json({error: "Not all necessery data provided"})
        let role = "user"
...
app.post('/api/books/:id/reviews', auth, (req, res) => {
    try {
        ...
        if(rating > 5 || rating < 1 || !Number.isInteger(Number(rating))) return res.status(403).json({error: "Incorrect data"})
        
        const query = db.prepare(`
            INSERT INTO reviews (bookId, userId, rating, comment) VALUES (?,?,?,?)`)
        const info = query.run(id, req.user.id, rating, reviewComment)
